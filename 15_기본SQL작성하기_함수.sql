...
SELECT INSTR(name, 'J') FROM score;  -- 대문자 J의 위치를 반환한다. 없으면 0을 반환한다.
...
SELECT CONCAT(CONCAT(name, ' '), kor) FROM score;  -- CONCAT()의 인수는 2개만 가능
...
SELECT TRIM(name) FROM score;  -- 양쪽 공백을 제거
...
SELECT TRUNC(567.8989, -2) FROM DUAL;  -- 백의 자리로 절사  500
