...
-- 2. 부서(department_id)별로 department_id, department_name, 평균연봉(salary)을 조회하시오.
-- 1) 스칼라 서브쿼리
...
-- 5. Patrick Sully(first_name: Patrick, last_name: Sully)와 같은 부서(department_id)에 근무하는 모든 사원정보를 조회하시오.
-- first_name : Patrick
-- last_name : Sully
...
-- 11. 부서번호(department_id)가 30인 사원들 중에서 부서번호가 50인 사원들의 최대연봉(salary)보다 더 많은 연봉을 받는 사원들을 조회하시오.
-- 1) 다중 행 서브쿼리
SELECT last_name
     , salary
  FROM employees
